(function ($) {
    function UserRolesKoModel() {
        this.Admin = ko.observable(false);
        this.Developer = ko.observable(false);
    };

    function UserKoModel() {
        var that = this;
        that.Email = ko.observable('');
        that.Department = ko.observable('');
        ...
        that.Roles = new UserRolesKoModel();
    };

    /***
     * User Data Store implementation.
     */
    function UserSlickStore() {
        // private
        var PAGESIZE = 50;
        var data = { length: 0 };
        var searchstr = "";
        ...
        function ensureData(from, to) {
            ...
            var url = "work/users?detail=true&filter[fields][type][]=submission&q=" + searchstr + "&start=" + (fromPage * PAGESIZE) + "&limit=" + (((toPage - fromPage) * PAGESIZE) + PAGESIZE);
        ...
        function onSuccess(resp) {
            ...
            data.length = Math.min(parseInt(resp.total), 1000); // limitation of the API
    ...
    $.extend(true, window, {
        Scope: {
            User: UserKoModel,
            UserRoles: UserRolesKoModel,
            UserStore: UserSlickStore
        }
    });
